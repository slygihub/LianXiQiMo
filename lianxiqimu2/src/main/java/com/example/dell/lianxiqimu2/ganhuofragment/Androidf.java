...
import com.example.dell.lianxiqimu2.adapter.GanhuoAdapter;
 ...
 */
public class Androidf extends Fragment {
    ...
    private void initView(View inflate) {
        ...
        list = new ArrayList<>();
        adapter = new GanhuoAdapter(list, getActivity());
        rc.setAdapter(adapter);

        Retrofit retrofit = new Retrofit.Builder().addCallAdapterFactory(RxJava2CallAdapterFactory.create()).addConverterFactory(GsonConverterFactory.create()).baseUrl(Servers.url1).build();
        Servers servers = retrofit.create(Servers.class);
        Observable<AndroidBean> androiddata = servers.androiddata();
        androiddata.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<AndroidBean>() {
                    ...
                    public void onNext(AndroidBean androidBean) {
                        List<AndroidBean.ResultsBean> list1 = androidBean.getResults();
                        list.addAll(list1);
